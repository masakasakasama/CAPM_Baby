...
export const MOCK_DURATION_MINUTES = 36;
...
export function selectMockExamQuestions(
  ...
  const selected = Object.entries(MOCK_BLUEPRINT).flatMap(([unit, count]) =>
    shuffled(
      bank.filter((question) => question.unit === Number(unit)),
      random,
    ).slice(0, count),
  );
  ...
  if (selected.length !== MOCK_EXAM_SIZE) {
    throw new Error(
      `Mock blueprint requires ${MOCK_EXAM_SIZE} questions, selected ${selected.length}`,
    );
  }
