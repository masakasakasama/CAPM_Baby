...
export function parseProgress(value: unknown): Progress | null {
  ...
  if (value.lastActivity !== undefined) {
    ...
    if (unit !== undefined && unit !== "all" && (!Number.isInteger(unit) || Number(unit) < 1 || Number(unit) > 4)) return null;
...
export function calculateReadiness(progress: Progress, totalQuestions = 48, totalUnits = 4): ReadinessBreakdown {
  ...
  const completedDetailUnits = progress.completedUnits.filter(
    (unit) => unit >= 1 && unit <= totalUnits,
  ).length;
  const study = clamp(completedDetailUnits / Math.max(1, totalUnits) * 100);
...
export function calculateSchedule(progress: Progress, readiness: number, now = new Date(), totalQuestions = 48): ScheduleProgress | null {
